...
def preparePayload(
    ...
    header['X-Allow-Completion'] = m.adaptH(options.get('allowCompletion', dflt.DEFAULT_ALLOW_COMPLETION))
...
def getAudience(notes):
    ...
    if str(audienceFlag) != '1' and str(audienceFlag) != '2' and str(audienceFlag) != '3':
        Logger.warning('Unknown audience: force default ({})'.format(dflt.DEFAULT_AUDIENCE))
...
def getInvited(notes):
    ...
    if str(invitedFlag) != '0' and str(invitedFlag) != '1':
        Logger.warning('Unknown invited: force default ({})'.format(dflt.DEFAULT_INVITED))
...
def getPopular(notes):
    ...
    if str(popFlag) != '0' and str(popFlag) != '1':
        Logger.warning('Unknown popular level: force default ({})'.format(dflt.DEFAULT_POPULAR))
...
def getPeer(notes):
    ...
    if str(peerFlag) != '0' and str(peerFlag) != '1':
        Logger.warning('Unknown peer reviewing type: force default ({})'.format(dflt.DEFAULT_PEER))
...
def getProceedings(notes):
    ...
    if str(proFlag) != '0' and str(proFlag) != '1':
        Logger.warning('Unknown proceedings status: force default ({})'.format(dflt.DEFAULT_PROCEEDINGS))
...
def setIDS(inTree,data):
    ...
    if data.get('journal',None):
        ...
        if not data.get('halJournalId',None):
            idJ = getDataFromHAL(txtsearch=data.get('journal'),
                                 typeDB='journal',
                                 typeI='title',
                                 returnFields='docid,title_s')
            if not idJ:
                idJ = getDataFromHAL(textsearch=data.get('journal'),
                                     typeDB='journal',
                                     typeI='title_approx',
                                     returnFields='docid,title_s')
...
def getTypeDoc(typeDoc):
    ...
    if typeDoc == 'article' or typeDoc == 'journalarticle' or typeDoc == 'articlejournal' or typeDoc == 'art': #Article dans une revue
        return 'ART'
    elif typeDoc == 'articlereview' or typeDoc == 'review' or typeDoc == 'artrev' or typeDoc == 'articlesynthese': #Article de synthèse
        return 'ARTREV'
    elif typeDoc == 'datapaper' or typeDoc == 'paperdata': #Cdata paper
        return 'DATAPAPER'
    elif typeDoc == 'bookreview' or typeDoc == 'compterendulecture': #Compte rendu de lecture
        return 'BOOKREVIEW'
    elif typeDoc == 'comm' or typeDoc == 'conferencePaper' or typeDoc == 'communication' or typeDoc == 'conference': #Communication dans un congrés
        return 'COMM'
    elif typeDoc == 'poster': #poster de conference
        return 'POSTER'
    elif typeDoc == 'proceedings' or typeDoc == 'recueilcommunications': #Proceedings\/Recueil des communication
        return 'PROCEEDINGS'
    elif typeDoc == 'issue' or typeDoc == 'specialissue' or typeDoc == 'numerospecial': #Numéro spécial
        return 'ISSUE'
    elif  typeDoc == 'ouv' or typeDoc == 'book' or typeDoc == 'monograph' or typeDoc == 'ouvrage': #Ouvrage 
        return 'OUV'
        ...
        return 'SYNTOUV'
    elif typeDoc == 'dictionary' or typeDoc == 'dictionnaire' or typeDoc == 'encyclopedie': #Dictionnaire ou encyclopédie
        return 'DICTIONARY'
        ...
        return 'BLOG'
    elif typeDoc == 'notice' or typeDoc == 'noticedictionary' or typeDoc == 'noticeencyclopede': #Notice de dictionnaire ou d'encyclopedie
        return 'NOTICE'
        ...
        return 'OTHER'
    elif typeDoc == 'undefined' or typeDoc == 'prepublication' or typeDoc == 'documenttravail': #pré-publication/document de travail
        return 'UNDEFINED'
    elif typeDoc == 'preprint' or typeDoc == 'prepublication': #preprint/pre-publication
        return 'PREPRINT'
    elif typeDoc == 'workingpaper': # working paper
        return 'WORKINGPAPER'
    elif typeDoc == 'creport' or typeDoc == 'chapitrerapport' or typeDoc == 'chaptereport': #chapitre de rapport
        return 'CREPORT'
    elif typeDoc == 'report' or typeDoc == 'rapport': #rapport
        return 'REPORT'
    elif typeDoc == 'resreport' or typeDoc == 'rapportrecherche' or typeDoc == 'researchreport': #rapport de recherche
        return 'RESREPORT'
    elif typeDoc == 'techreport' or typeDoc == 'rapporttechnique' or typeDoc == 'technicalreport': #rapport technique
        return 'TECHREPORT'
    elif typeDoc == 'fundreport' or typeDoc == 'rapportcontrat' or typeDoc == 'rapportprojet' or typeDoc == 'contractreport' or typeDoc == 'projectreport': #rapport de contrat/projet
        return 'FUNDREPORT'
    elif typeDoc == 'expertreport' or typeDoc == 'rapportexpertise': #rapport d'une expertise collective
        return 'EXPERTREPORT'
    elif typeDoc == 'dmp' or typeDoc == 'plangestiondonnees': #data management plan/plan gestion de données
        return 'DMP'
        ...
        return 'IMG'
    elif typeDoc == 'photography' or typeDoc == 'photo' or typeDoc == 'photographie': #photographie
        return 'PHOTOGRAPHY'
        ...
        return 'NOTE'
    elif typeDoc == 'otherreport' or typeDoc == 'autrerapport': #autre rapport, séminaire...
        return 'OTHERREPORT'
...
def getStructType(name):
    ...
    if not name:
        ...
        if 'université' in name or 'university' in name or 'univ' in name or 'école' in name or 'school' in name:
            return 'institution'
